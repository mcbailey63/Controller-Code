...
class RobotiqGripper(object):
    ...
    def move_no_wait(self, pos_in_mm):
        """
        Start moving the gripper to a specified position in (mm) without
        waiting for motion to complete.

        Parameters:
            pos_in_mm (int | float): position in millimeters.

        Returns:
            True if the command succeeded, otherwise it returns False
        """
        return self.call("MOVE_NOWAIT", "rq_move_mm(" + str(pos_in_mm) + ")")
