"""Teleoperate a UR5 over RTDE with a XBOX controller.

Requirements:
- `pip install ur-rtde pygame`
- A XBOX controller paired to the computer

Default controls:
- Hold `RB` to enable robot motion (deadman switch)
- Left stick: translate TCP in robot base X / Y
- `RT` / `LT`: translate TCP +Z / -Z
- Right stick X: rotate about Z
- Right stick Y: rotate about Y
- D-pad left / right: rotate about X
- Hold `A`: open gripper gradually
- Hold `X`: close gripper gradually
- Tap `Y`: toggle fully open / fully closed
...
SINGULARITY_DAMPING_THRESHOLD = 0.05
SINGULARITY_DAMPING_EPSILON   = 0.005  

# Common XBOX button indices in pygame.
BUTTON_X = 2
BUTTON_A = 0
BUTTON_B = 1
BUTTON_Y = 3
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Teleoperate a UR5 over RTDE with a XBOX controller."
    )
    parser.add_argument(
        "--robot-ip",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Initialize and control a Robotiq gripper from the XBOX controller.",
    )
    parser.add_argument(
        "--disable-gripper",
        dest="enable_gripper",
...
def manipulability_scale(J: np.ndarray, threshold: float, epsilon: float) -> float:
    w = float(np.sqrt(max(0.0, np.linalg.det(J @ J.T))))
    if w >= threshold:
        return 1.0
    scale = w / (threshold + epsilon)
    return scale ** 2
...
def wait_for_controller() -> pygame.joystick.Joystick:
    if pygame.joystick.get_count() == 0:
        raise RuntimeError("No game controller found. Pair the XBOX controller first.")
...
def main() -> int:
    ...
    try:
        ...
        print(f"Singularity threshold: {SINGULARITY_DAMPING_THRESHOLD}")      
        print(
        ...
        if args.enable_gripper:
            print(
                f"Gripper controls: hold button {BUTTON_A} to open, "
                f"hold button {BUTTON_X} to close, "
                f"tap button {BUTTON_Y} to toggle full open/close."
            )
        ...
        while True:
            ...
            trigger_up = trigger_to_unit_range(axis(joystick, AXIS_RT))
            trigger_down = trigger_to_unit_range(axis(joystick, AXIS_LT))
            dpad_roll = dpad_x(joystick)
            open_pressed = button(joystick, BUTTON_A)
            close_pressed = button(joystick, BUTTON_X)
            toggle_pressed = button(joystick, BUTTON_Y)
            now = time.time()
            ...
            try:
                ...
                J = jac_end_effector(np.array(q_actual))   # your FK-based Jacobian
                w = float(np.sqrt(max(0.0, np.linalg.det(J @ J.T))))
                m_scale = manipulability_scale(J, SINGULARITY_DAMPING_THRESHOLD, SINGULARITY_DAMPING_EPSILON)
                tcp_twist_damped = [v * m_scale for v in tcp_twist]

                if enabled:
                    rtde_c.speedL(tcp_twist_damped, args.acceleration * m_scale, CONTROL_PERIOD_S)
                else:
                    rtde_c.speedStop()
